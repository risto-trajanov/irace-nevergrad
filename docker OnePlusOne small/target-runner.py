        ...
        if name == 'CMA-ES':
            ...
            while cand_params:
                ...
                if param == "--noise_handling":
                    ...
                    target_runner_error("unknown parameter %s" % (param))
            optimizer = ParametrizedOnePlusOne(noise_handling=noise, mutation=mutation, crossover=crossover, rotation=self.rotation, use_pareto=use_pareto).set_name('OnePlusOne_my', register=True)
        elif name == "Scipy":
            while cand_params:
                param = cand_params.pop(0)
                value = cand_params.pop(0)
